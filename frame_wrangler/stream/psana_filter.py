...
def build_event_code_map(experiment: str, runs: list[str], code: int) -> set[int]:
    """
    Query psana for all events across the given runs and return the set of
    ...
    runs:
        List of run numbers (as strings) to query.
    code:
    ...
    active: set[int] = set()
    for run_num in runs:
        ds = DataSource(exp=experiment, run=int(run_num), detectors=["timing"])
        myrun = next(ds.runs())
...
def make_psana_filter(experiment: str, runs: list[str], code: str):
    """
    ...
    """
    active = build_event_code_map(experiment, runs, int(code))
